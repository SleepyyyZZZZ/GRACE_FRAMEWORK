**Rules for Interacting with Code Tagged via Semantic Template (for Claude Code Agent)**
...
*   **3. Semantic Verification Gate (MANDATORY)**
    *   After ANY code change (new file, edit, refactor, bug fix), run: `python3 tools/check_semantics.py`. **Zero ERRORs is a hard gate** before task completion. Fix all errors, review warnings.

*   **4. Cognitive Alignment and Trust**
...
Any agent action within a phase WITHOUT loading its protocol via the corresponding skill is a critical rule violation (**CRITICAL_RULE_VIOLATION**). The model is forbidden to rely on its memory in these matters.

**1. "Architect" Phase (Design and Planning)**
*   **TRIGGER:** Receiving a new task requiring code writing, refactoring, or adding a feature.
*   **MANDATORY ACTION:** Load the `mode-architect` skill.
*   **GOAL:** Explore the solution space, create a plan file in `plans/active/` (naming: `YYYY-MM-DD_short_description.md`, format: `$START_DEV_PLAN` / `$END_DEV_PLAN`), and superposition hypotheses.
...
*   **MANDATORY ACTION:** Load the `mode-code` skill.
*   **GOAL:** 100% implementation of logic with semantic markup, SFT priming, Anti-Loop protection in tests, and mandatory AppGraph update (local module graph + global `docs/AppGraph.xml`).
...
*   **MANDATORY ACTION:** Load the `mode-debug` skill.
...
*   **MANDATORY ACTION:** Load the `mode-qa` skill.
*   **GOAL:** Impartial verification of results and formation of a structured Bug Report.

**PROTOCOL FOR CALLING TEST SUBAGENTS**
If subagents are required for task verification (via the Agent tool), they must be explicitly instructed (in the `prompt` text): "Load the `mode-qa` skill and perform testing according to its instructions."
...
*   **Path 1: From Graph to Code.** Start with `AppGraph.xml`. Identify the target module. Use the Read tool. Study the `MODULE_MAP` inside the file. Use the Grep tool to find `START_FUNCTION_...`.
*   **Path 2: From Log to Code.** Given a log with `[BLOCK_NAME]`, use the Grep tool by `FunctionName` and `BLOCK_NAME` for an instant jump to the epicenter.
*   **Path 3: Semantic Search.** When searching, formulate queries as a dense set of terms rather than sentences (e.g.: "UserSession Redis auth login KEYWORDS" instead of "where does the login occur").

**2. Modification Tools and Safety**
Use the Edit tool for pinpoint edits.

*   **Working Principle:** The tool performs an exact replacement of a text fragment (`old_string`) with a new one (`new_string`).
*   **"Read before edit" Rule:** You MUST call the Read tool for the file before using Edit to get the exact text and indentation.
*   **Use of Anchors:** If errors occur (e.g., multiple matches of `old_string`), it is recommended to expand the search area by including unique semantic anchors `# START_BLOCK` / `# END_BLOCK` in `old_string`.
...
*   **After every modification, run `python3 tools/check_semantics.py` and fix all ERRORs before proceeding.**

*   **4. AppGraph Maintenance (TWO-LEVEL MANDATORY)**
    *   **Rule:** Every new or modified module directory MUST have a local `AppGraph.xml` covering ONLY its own entities.
    *   **Bridge Rule:** After any local graph creation or update, `docs/AppGraph.xml` (global) MUST be updated with a `<MODULE_LOCAL FILE="path/to/module/AppGraph.xml" TYPE="LOCAL_GRAPH_REF">` link pointing to the local graph. An orphaned local graph (not referenced from global) is a **GRAPH_VIOLATION**.
    *   **Trigger:** Any new file created, any module modified, any Code Phase completion.
    *   **Tool:** Use the `graph-protocol` skill for proper XML structure.
