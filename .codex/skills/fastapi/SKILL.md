...
### Thin Controllers (Critical Rule)

Routes ONLY do 3 things: validate input, call service, return DTO. No business logic in routes.

```python
# BAD — business logic leaked into route
@router.post("/orders")
async def create_order(data: OrderCreate, db: AsyncSession = Depends(get_db)):
    # checking inventory in route — WRONG
    inventory = await db.execute(select(Inventory).where(...))
    if inventory.scalar().quantity < data.quantity:
        raise HTTPException(400, "Not enough inventory")
    order = Order(**data.model_dump())
    order.total = data.price * data.quantity * 0.9  # discount logic in route — WRONG
    db.add(order)
    await db.commit()
    return order
...
async def create_order(
    ...
    """Side effects: NONE (delegated to service)"""
...
2. **Never put business logic in routes** - Use service layer (see Thin Controllers above)
