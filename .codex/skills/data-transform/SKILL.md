...
**Principle:** This guide describes optimal strategies for data transformation tasks, especially ETL processes using SQL and Python/Pandas.
...
*   **Limitations:**
    ...
    *   **Loops:** Explicit loops over DataFrame rows (`for index, row in df.iterrows():`) are **catastrophically slow**. Even worse is executing a SQL query inside such a loop (N+1 queries problem).
...
*   **Normalization and Data Types:**
    ...
    *   **Use `astype('category')`:** Convert low-cardinality string columns to categories to save memory and speed up calculations.
...
*   **Exporting Data to DB (Staging Table Pattern):**
    1.  Form result DataFrame with `EditState` column (`'I'`, `'U'`, `'D'`).
    2.  Export in **one** transaction to staging table using `df.to_sql()`.
    3.  Execute SQL to atomically apply changes to main table.
    4.  **Persistence Rule:** Do not delete staging table. Clear at **beginning** of next run.

#### 3. "Pandas-like" SQL Technique via Intermediate Tables

*   **Concept:** Use SQL in **procedural style**, emulating step-by-step Pandas logic. More performant on large datasets.
*   **Practice:**
    1.  **Decompose complex SELECTs:** Create series of intermediate tables (`CREATE TABLE ... AS SELECT ...`).
